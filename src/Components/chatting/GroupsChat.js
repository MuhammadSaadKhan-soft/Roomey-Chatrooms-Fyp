...
const GroupsChat = ({ roomName, themes }) => {
...
const handleNotificationCheck = () => {
  ...
  // Button click handl
  return (
    <Container fluid className={`vh-100 ${themes ? 'bg-black text-light' : 'bg-white text-dark'}`}>
      <Row  className={themes?'dark-mode':'light-mode'}>
        {/* Sidebar */}
        <Col md={3} className={` ${themes?'dark-mode':'light-mode'}border-end p-3 d-none d-md-block  overflow-auto `}>

{/* Main User Profile Card */}
<Card className={`${themes ? 'bg-black text-light' : 'bg-white text-dark'} mb-4 text-center`}>
  {profilePicture && (
    <Card.Img
      variant="top"
      src={profilePicture}
...
<Card className={` ${themes?'dark-mode':'light-mode'} mb-4`}>
  <Card.Body>
    <p className={` ${themes?'dark-mode':'light-mode'} mb-2`}><strong>Admin:</strong> <span className="text-muted">{admin}</span></p>
    <p className={` ${themes?'dark-mode':'light-mode'} mb-0`}><strong>Users Count:</strong> <span className="text-muted">{userCount}</span></p>
  </Card.Body>
</Card>

{/* User List */}
<Card className={themes?'dark-mode':'light-mode'}>
    ...
    {userList.length > 0 ? (
      ...
      <li className="list-group-item text-muted">Waiting for users...</li>
        ...
        <Col md={9} className="d-flex flex-column p-0">
          {/* Header */}
          <div className={`d-flex justify-content-between align-items-center p-3 border-bottom ${themes  ? 'bg-secondary' : 'bg-white'}`}>
          ...
          {/* Messages */}
          <div className={`flex-grow-1 overflow-auto p-3 ${themes ?'dark-mode':'light-mode'}` }>
            {loading ? (
              <div>Loading...</div>
            ) : (
              messages.map((item) => (
                ...
                >
                 <div className={`p-2 rounded ${item.sender === user 
